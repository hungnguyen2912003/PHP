...
@section('title', __('title.create_weight'))

@section('content')
    <div class="main-content-container overflow-hidden">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-4 mt-1">
            <h3 class="mb-0">
                {{ __('title.create_weight') }}
            </h3>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb align-items-center mb-0 lh-1">
                    ...
                    <li aria-current="page" class="breadcrumb-item active">
                        <span>{{ __('breadcrumb.weight_management') }}</span>
                    </li>
                    <li aria-current="page" class="breadcrumb-item active">
                        <span class="text-secondary">{{ __('title.create_weight') }}</span>
                    </li>
                </ol>
            </nav>
        </div>

        <form id="addWeightForm" action="{{ route('client.weight.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="row">
                <div class="col-lg-12">
                        ...
                        <div class="row">
                                ...
                                <div class="mb-20">
                                    ...
                                    <div class="form-floating">
                                        <input class="form-control" id="weight" name="weight"
                                            ...
                                            value="{{ old('weight') }}" />
                                        <label for="weight"><i
                                                class="material-symbols-outlined fs-16">weight</i>{{ __('placeholder.weight') }}</label>
                                    </div>
                                    @error('weight')
                            ...
                            <div class="col-lg-12">
                                <div class="mb-20">
                                    ...
                                    <div class="form-floating">
                                        <textarea class="form-control" id="notes" name="notes" placeholder="{{ __('placeholder.notes') }}" style="height: 100px">{{ old('notes') }}</textarea>
                                        <label for="notes"><i class="ri-sticky-note-line"></i>{{ __('placeholder.notes') }}</label>
                                    </div>
                                    @error('notes')
                                        <div class="text-danger mt-1">{{ $message }}</div>
                                    @enderror
