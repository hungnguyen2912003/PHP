...
@section('content')
    ...
    <form id="common-form" action="{{ route('client.password.update', $token) }}" method="POST">
        @csrf
        <div class="mb-20">
            ...
            <div class="form-group" id="password-show-hide">
                <div class="password-wrapper position-relative password-container form-floating">
                    <input class="form-control text-secondary password @error('password') is-invalid @enderror"
                        placeholder="{{ __('placeholder.new_password') }}" type="password" name="password" id="password" />
            ...
            <div class="form-group" id="password-show-hide-confirm">
                <div class="password-wrapper position-relative password-container form-floating">
                    <input class="form-control text-secondary password @error('password_confirmation') is-invalid @enderror"
                        placeholder="{{ __('placeholder.new_password_confirmation') }}" type="password"
                        name="password_confirmation" id="password_confirmation" />
                    <label for="password_confirmation"><i class="ri-lock-line"></i>
                        {{ __('placeholder.new_password_confirmation') }}</label>
                    <i aria-hidden="true"
                        class="ri-eye-off-line password-toggle-icon translate-middle-y top-50 position-absolute cursor text-secondary"
                        style="color: #A9A9C8; font-size: 22px; right: 15px; z-index: 10;">
                    </i>
