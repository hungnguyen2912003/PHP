...
<form id="resetPasswordForm" action="{{ route('client.password.update', $token) }}" method="POST">
    @csrf
    <div class="mb-20">
        ...
        <div class="form-floating">
            ...
            <label for="username"><i class="ri-user-line"></i> {{ __('placeholder.username') }}</label>
        </div>
        @error('username')
    ...
    <div class="mb-20">
        ...
        <div class="form-floating">
            ...
            <label for="email"><i class="ri-mail-line"></i> {{ __('placeholder.email') }}</label>
        </div>
        @error('email')
            <span class="text-danger">{{ $message }}</span>
        @enderror
    ...
    <div class="mb-20">
        ...
        <div class="form-group" id="password-show-hide">
            <div class="password-wrapper position-relative password-container form-floating">
                <input class="form-control text-secondary password @error('password') is-invalid @enderror" placeholder="{{ __('placeholder.new_password') }}"
                    type="password" name="password" id="password" />
                <label for="password"><i class="ri-lock-line"></i> {{ __('placeholder.new_password') }}</label>
                <i aria-hidden="true"
    ...
    <div class="mb-20">
        <label class="label fs-16 mb-2">
            {{ __('label.new_password_confirmation') }} <span class="text-danger">*</span>
        </label>
        <div class="form-group" id="password-show-hide-confirm">
            <div class="password-wrapper position-relative password-container form-floating">
                <input class="form-control text-secondary password @error('password_confirmation') is-invalid @enderror" placeholder="{{ __('placeholder.new_password_confirmation') }}"
                    type="password" name="password_confirmation" id="password_confirmation" />
                <label for="password_confirmation"><i class="ri-lock-line"></i> {{ __('placeholder.new_password_confirmation') }}</label>
                <i aria-hidden="true"
                    class="ri-eye-off-line password-toggle-icon translate-middle-y top-50 position-absolute cursor text-secondary"
                    style="color: #A9A9C8; font-size: 22px; right: 15px; z-index: 10;">
                </i>
    ...
    <div class="mb-20">
        <button class="btn btn-primary fw-normal text-white w-100" id="submitBtn" data-processing-text="{{ __('notification.processing') }}"
            style="padding-top: 18px; padding-bottom: 18px;" type="submit">
            <span id="btnText">{{ __('auth.reset_password_btn') }}</span>
