<div class="d-flex align-items-center justify-content-end gap-1">
    <a href="{{ route('admin.weights.show', $weight->id) }}" class="btn btn-sm btn-icon border-0 p-0 text-body bg-transparent"
    ...
    <a href="{{ route('admin.weights.edit', $weight->id) }}" class="btn btn-sm btn-icon border-0 p-0 text-body bg-transparent"
        data-bs-toggle="tooltip" data-bs-placement="top" title="{{ __('button.edit') }}">
        <span class="material-symbols-outlined fs-20">edit</span>
    </a>
    <form action="{{ route('admin.weights.destroy', $weight->id) }}" method="POST" class="d-inline">
        @csrf
        @method('DELETE')
