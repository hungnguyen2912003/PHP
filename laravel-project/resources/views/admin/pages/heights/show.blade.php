...
@section('title', __('title.height_details'))

@section('content')
    <div class="main-content-container overflow-hidden">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-4 mt-1">
            <h3 class="mb-0">
                {{ __('title.height_details') }}
            </h3>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb align-items-center mb-0 lh-1">
                    ...
                    <li aria-current="page" class="breadcrumb-item active">
                        <a class="text-decoration-none" href="{{ route('admin.heights.index') }}">
                            <span class="text-body fs-14 hover">{{ __('breadcrumb.height_list') }}</span>
                        </a>
                    </li>
                    <li aria-current="page" class="breadcrumb-item active">
                        <span class="text-secondary">{{ __('title.height_details') }}</span>
                    </li>
                </ol>
            </nav>
        </div>
            ...
            <div class="row g-3">
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <div class="form-control py-3">
                                <i class="material-symbols-outlined fs-16 me-1 align-middle">account_circle</i>
                                {{ $height->user->fullname ?? __('value.unknown') }}
                            </div>
                        </div>
                        <div class="col-md-6">
                            <label class="label fs-16 mb-2">{{ __('label.height') }} (cm)</label>
                            <div class="form-control py-3">
                                <i class="material-symbols-outlined fs-16 me-1 align-middle">height</i>
                                {{ $height->height }} cm
                            </div>
                        </div>
                        <div class="col-md-6">
                            ...
                            <div class="form-control py-3">
                                ...
                                {{ $height->recorded_at->format(app()->getLocale() == 'vi' ? 'd/m/Y H:i' : (app()->getLocale() == 'ja' ? 'Y/m/d H:i' : 'Y-m-d H:i')) }}
                            </div>
                        </div>
                        <div class="col-12">
                            ...
                            <div class="form-control" style="min-height: 150px;">
                                ...
                                {!! nl2br(e($height->notes)) ?: '<span class="text-muted">' . __('value.not_available') . '</span>' !!}
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4">
                    ...
                    <div class="border rounded-3 p-3 h-100 d-flex flex-column align-items-center justify-content-center">
                        @if ($height->attachment_url)
                            <a href="{{ asset($height->attachment_url) }}" target="_blank">
                                <img src="{{ asset($height->attachment_url) }}" alt="Attachment" class="img-fluid rounded shadow-sm"
                ...
                <div class="col-12">
                    <div class="d-flex gap-2 justify-content-center mt-4">
                        <a href="{{ route('admin.heights.edit', $height->id) }}"
                            class="btn btn-primary fw-normal text-white">
                            <i class="ri-edit-line"></i> {{ __('button.edit') }}
                        </a>
                        <a href="{{ route('admin.heights.index') }}" class="btn btn-danger fw-normal text-white">
                            {{ __('button.back') }}
                        </a>
                    </div>
                </div>
